...
시연이 급하게 필요한 상황을 가정해, 문서 등록/관리 API까지 구현하기보다 고객사가
관심을 가진 "내부 문서 기반 답변" 흐름을 빠르게 보여주는 데 집중했습니다.

현재는 이 프로젝트 자체를 고객사에 제공할 챗봇 서비스라고 가정하고, 해당 서비스 설명
문서 1개를 앱 기동 시 seed합니다. 사용자가 서비스 기능을 질문하면 관련 chunk를 찾아
AI 요청 context에 포함하고, 응답에는 참고 문서 정보가 `sources`로 반환됩니다. 이는
문서를 모델에 학습시키는 fine-tuning이 아니라, 시연용 최소 RAG 흐름입니다.
