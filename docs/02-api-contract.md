...
```json
{
  ...
  "useKnowledgeBase": true      // 선택; 기본 true (Demo Knowledge Base 사용)
}
```
응답 `201` (비스트리밍):
```json
...
### 2.1.2 Demo Knowledge Base 사용
MVP1의 문서 기반 답변은 **문서 등록 API가 없는 시연용 RAG 흐름**입니다. 서버는
현재 챗봇 서비스 설명 문서를 demo knowledge로 사전 등록하고, `useKnowledgeBase`가
`true`일 때 질문과 관련된 chunk를 검색해 AI 요청 context에 포함합니다.

이는 fine-tuning이나 모델 학습이 아닙니다. 등록된 문서 조각을 검색해 prompt context로
전달하는 방식입니다. demo knowledge가 없거나 관련 chunk가 없으면 기존 일반 chat처럼
동작합니다.

`useKnowledgeBase` 기본값은 데모 설득력을 위해 `true`입니다. 문서 chunk가 존재하는 경우
일반 채팅에도 질문 embedding 생성과 chunk similarity 계산 비용이 추가됩니다. 단, 현재
embedding model과 일치하는 chunk가 없으면 질문 embedding 호출 없이 일반 chat으로
fallback합니다.
...
```json
{
  ...
  "model": "gpt-4o-mini",
  "createdAt": "2026-06-27T05:03:00Z",
  "sources": [
    {
      "documentTitle": "AIChatbot 서비스 설명",
      "chunkIndex": 1
    }
  ]
}
...
prompt 구성 원칙:
- system message는 문서 context가 있으면 우선 참고하도록 지시합니다.
- context에 없는 내용은 추측하지 않고 확인되지 않는다고 답하도록 지시합니다.
- 같은 스레드의 이전 대화는 기존 방식대로 history에 포함합니다.
- 문서 context는 topK chunk만 포함합니다.
