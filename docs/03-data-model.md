...
demo_documents (1) ───< (N) document_chunks (1) ───< (N) chat_document_sources >─── (1) chats
```
...
- `demo_documents`와 `document_chunks`는 문서 등록 API가 없는 시연용 knowledge base.
- `chat_document_sources`는 특정 채팅 답변에 참고된 chunk metadata를 기록.
...
### 2.6 `demo_documents`
MVP1 시연용 문서 기반 답변을 위한 사전 등록 문서입니다. API로 등록/삭제하지 않고,
애플리케이션 시딩 또는 Flyway seed로 현재 챗봇 서비스 설명 문서를 넣습니다.

| 컬럼 | 타입 | 제약 | 비고 |
|------|------|------|------|
| `id` | BIGINT | PK, identity | |
| `title` | VARCHAR(255) | NOT NULL | 예: `AIChatbot 서비스 설명` |
| `source_key` | VARCHAR(100) | NOT NULL, UNIQUE | seed 중복 방지용 안정 키 |
| `created_at` | TIMESTAMPTZ | NOT NULL, 기본 now() | |
| `deleted_at` | TIMESTAMPTZ | NULL 허용 | 현재 삭제 API 없음 |

인덱스:
- `UNIQUE(source_key)` — 같은 demo document 중복 seed 방지.
- `idx_demo_documents_deleted_at` on (`deleted_at`) — 삭제 제외 조회.

### 2.7 `document_chunks`
demo document를 검색 가능한 단위로 나눈 chunk입니다. embedding은 PostgreSQL 기본
환경에서 동작하도록 `TEXT` 또는 JSON 문자열로 저장합니다. `pgvector`는 사용하지 않습니다.

| 컬럼 | 타입 | 제약 | 비고 |
|------|------|------|------|
| `id` | BIGINT | PK, identity | |
| `document_id` | BIGINT | NOT NULL, FK → `demo_documents(id)` | 원문 문서 |
| `chunk_index` | INT | NOT NULL | 문서 내 0-base 또는 1-base 순번. 구현에서 하나로 고정 |
| `content` | TEXT | NOT NULL | AI context에 포함할 plain text |
| `embedding` | TEXT | NULL 허용 | embedding vector를 JSON 문자열로 저장 |
| `embedding_model` | VARCHAR(100) | NULL 허용 | seed 시 사용한 embedding model/provider 식별자 |
| `created_at` | TIMESTAMPTZ | NOT NULL, 기본 now() | |
| `deleted_at` | TIMESTAMPTZ | NULL 허용 | 현재 삭제 API 없음 |

제약/인덱스:
- `UNIQUE(document_id, chunk_index)` — 문서 내 chunk 순번 중복 방지.
- `idx_document_chunks_document_id` on (`document_id`)
- `idx_document_chunks_deleted_at` on (`deleted_at`)
- `idx_document_chunks_embedding_model` on (`embedding_model`)

검색 방식:
- 현재 `EmbeddingClient.model`과 같은 `embedding_model`의 삭제되지 않은 chunk를 조회.
- 대상 chunk가 있으면 질문 embedding 생성.
- 애플리케이션에서 cosine similarity 계산.
- topK chunk를 prompt context로 포함.
- 모델이 바뀌면 앱 시딩 단계에서 기존 chunk의 embedding을 재생성한다.

### 2.8 `chat_document_sources`
채팅 답변에 어떤 document chunk가 참고되었는지 저장합니다. 채팅 목록/상세 응답에서
`sources`를 다시 보여주기 위한 metadata입니다.

| 컬럼 | 타입 | 제약 | 비고 |
|------|------|------|------|
| `id` | BIGINT | PK, identity | |
| `chat_id` | BIGINT | NOT NULL, FK → `chats(id)` | 답변 채팅 |
| `document_chunk_id` | BIGINT | NOT NULL, FK → `document_chunks(id)` | 참고 chunk |
| `document_title` | VARCHAR(255) | NOT NULL | 응답 안정성을 위한 제목 snapshot |
| `chunk_index` | INT | NOT NULL | 응답 안정성을 위한 chunk index snapshot |
| `score` | DOUBLE PRECISION | NULL 허용 | cosine similarity 점수. 응답에는 기본 미노출 |
| `created_at` | TIMESTAMPTZ | NOT NULL, 기본 now() | |

인덱스:
- `idx_chat_document_sources_chat_id` on (`chat_id`)
- `idx_chat_document_sources_chunk_id` on (`document_chunk_id`)
...
| demo knowledge chunk 전체 조회 후 앱 내 similarity 계산 | 문서 기반 답변 데모 | `idx_document_chunks_deleted_at`, `idx_document_chunks_document_id` |
| 채팅별 참고 source 조회 | 채팅 생성 응답/목록 응답 | `idx_chat_document_sources_chat_id` |

---

## 5. 마이그레이션 노트

- `V1__init.sql` — 기본 5개 테이블(users, chat_threads, chats, feedbacks,
  ...
  H2 호환 형태로 작성).
- `V2__demo_knowledge_base.sql` — demo_documents, document_chunks,
  chat_document_sources 테이블만 생성. 서비스 설명 seed 문서, chunk, embedding은
  provider/API key/embedding model에 의존하므로 애플리케이션 시딩으로 생성한다.
...
| `demo_documents` | `DemoDocument` | `knowledge` |
| `document_chunks` | `DocumentChunk` | `knowledge` |
| `chat_document_sources` | `ChatDocumentSource` | `knowledge` 또는 `chat` |
